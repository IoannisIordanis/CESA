...
class GradientBoostingSurvival(SupervisedMethodInterface):
    def __init__(self, event_preferences: EventPreferences, seq_length=10, normalize=False, u_sample_rate=1,save_model=False, *args, **kwargs):
    ...
    def fit(self, historic_data: list[pd.DataFrame], historic_sources: list[str], event_data: pd.DataFrame,
        ...
        for current_historic_data, current_historic_source, labels in zip(historic_data, historic_sources,
            ...
            ydf=pd.DataFrame({'event': [lb for lb in e_final],'RUL': [max(1, lb) for lb in y_final]})
            fit_data = pd.DataFrame(X_final)
            if self.sample_rate<1:
                ydf,fit_data=self.undersample_by_rul( ydf, fit_data, sample_rate=self.sample_rate)
